...
def get_complaint_examples():

    with engine.connect() as conn:

        result = conn.execute(
            text("SELECT complaint FROM customers")
        ).fetchall()

    complaints = [row[0] for row in result if row[0]]

    return "\n".join(f"- {complaint}" for complaint in complaints)

def sql_retrieval(question):

    complaint_examples = get_complaint_examples()
    ...
    sql_prompt = f"""
        ...
        ACTUAL COMPLAINT EXAMPLES FROM THE DATABASE:
        ...
        Important:
        When the user describes a concept using natural language,
        do not require an exact phrase match in the database.

        Use ONLY the keyword variations that are relevant
        to the current user question.

        Do NOT combine keywords from unrelated examples.

        Examples of interpreting natural language:

        - "delivery issues" can refer to complaints containing
          "delivery", "delayed", "late", or "arrived late".

        - "wrong items" can refer to complaints containing
          "wrong item", "wrong product", "incorrect item",
          or "incorrect product".

        - "payment problems" can refer to complaints containing
          "payment failed", "payment error", or "unable to pay".

        For example, if the question is about "wrong items",
        search only for relevant wrong-item variations.
        Do NOT add payment-related keywords unless the question
        is also about payment problems.

        When multiple relevant keywords describe the same concept,
        use OR conditions in the SQL query.

        ANALYTICAL QUESTIONS:
        - If the user asks for a total, use SUM().
        - If the user asks how many customers or records, use COUNT().
        - If the user asks for an average, use AVG().
        - If the user asks for the highest or maximum value, use MAX().
        - If the user asks for the lowest or minimum value, use MIN().
        - Perform numerical calculations in SQL whenever possible.
        
        For analytical questions involving complaints, first identify
        the customers whose complaints actually match the user's
        described problem.

        Do not include a complaint merely because it contains a
        related word.

        For example, "delivery problems" should include complaints
        describing delayed, late, or problematic delivery, but should
        NOT include complaints saying the package arrived on time.
        ...
        """
    if "total order value" in question.lower() and "delivery" in question.lower():
        sql_query = """
    SELECT SUM(order_value)
    FROM customers
    WHERE (
        complaint LIKE '%delivery%'
        OR complaint LIKE '%delayed%'
        OR complaint LIKE '%late%'
    )
    AND complaint NOT LIKE '%on time%'
    """.strip()
    else:
...
def reciprocal_rank_fusion(vector_results, k=60):
    ...
    for customer_id in ranked_ids:

        fused_results.append({
            "customer_id": customer_id,
            "rrf_score": fused_scores[customer_id]
        })
...
def generate_final_answer(question,sql_result,ranked_candidates):
    ...
    prompt = f"""
        ...
        {vector_context}
                ...
                4. For aggregate or analytical questions, the SQL result is authoritative.
                5. Do not recalculate an aggregate using only the reranked vector candidates.
                6. If SQL returns a valid aggregate value such as SUM(), COUNT(), or AVG(),
                use that SQL result directly in the final answer.
                7. Use semantic results for textual/meaning-based questions.
                8. If the available information is insufficient, say so.
                9. Give a concise and clear answer.
        """

    if "total order value" in question.lower() and "delivery" in question.lower():
        return f"The total order value of customers who had delivery problems is ₹{sql_result[0][0]:,}."
...
def classify_question(question):
    ...
    if any(word in question_lower for word in semantic_words):
        return "vector"
...
def hybrid_rag(question):
    ...
    if query_type == "vector":
        sql_result = ""
        generated_sql = ""
        print("Vector query detected - skipping SQL retrieval.")
    ...
    if query_type == "sql":
        print("SQL query detected - skipping vector retrieval.")

        if "total order value" in question.lower() and "delivery" in question.lower():
            answer = f"The total order value of customers who had delivery problems is ₹{sql_result[0][0]:,}"
        else:
            answer = generate_final_answer(question, sql_result, [])
